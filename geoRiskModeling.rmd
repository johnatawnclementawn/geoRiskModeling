...
```{r pullMetro, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
# Code originally from TOD Assignment
...
thefts <- st_read("https://opendata.arcgis.com/datasets/6af5cb8dc38e4bcbac8168b27ee104aa_38.geojson") %>%
  st_transform(crs) %>%
	...
	mutate(Legend = "Theft", Count = 1) %>%
	dplyr::select(Legend, Count) 


allPredictors <- rbind(abandonedCars, dumping, liquourLicenses, streetlights, thefts)
```
...
vars_net <- vars_net %>%
    mutate(Abandoned_Cars.nn = nn_function(st_c(st_coid(vars_net)), 
                                           st_c(abandonedCars),
                                           k = 5)) %>%
    mutate(Dumping.nn = nn_function(st_c(st_coid(vars_net)), 
                                           st_c(dumping),
                                           k = 5)) %>%
    mutate(liquourLicenses.nn = nn_function(st_c(st_coid(vars_net)), 
                                           st_c(liquourLicenses),
                                           k = 5)) %>%
    mutate(streetlights.nn = nn_function(st_c(st_coid(vars_net)), 
                                           st_c(streetlights),
                                           k = 5)) %>%
    mutate(thefts.nn = nn_function(st_c(st_coid(vars_net)), 
                                           st_c(thefts),
                                           k = 5))
...
ggplot() +
      ...
      labs(title="Nearest Neighbor Risk Factors") +
      mapTheme()
```
...
do.call(grid.arrange,c(varList, ncol = length(vars), top = "Local Morans I statistics, Burglary"))
```
