...
ggplot() + 
  geom_sf(data = DCBoundary) +
  geom_sf(data = dumping, colour="red", size=0.1, show.legend = "point") +
  labs(title= "Illegal Dumping,\nWashington DC - 2017") +
  mapTheme(title_size = 14),

ggplot() + 
  geom_sf(data = DCBoundary, fill = "grey40") +
  stat_density2d(data = data.frame(st_coordinates(dumping)), 
                 aes(X, Y, fill = ..level.., alpha = ..level..),
                 size = 0.01, bins = 40, geom = 'polygon') +
  scale_fill_viridis() +
  scale_alpha(range = c(0.00, 0.35), guide = FALSE) +
  labs(title = "Density of Illegal Dumping\nWashington DC - 2017") +
  mapTheme(title_size = 14) + theme(legend.position = "none"))
...
###### Illegal dumping #######
crime_net <- 
  dplyr::select(dumping) %>% 
  mutate(countDumping = 1) %>% 
  aggregate(., fishnet, sum) %>%
  mutate(countDumping = replace_na(countDumping, 0),
         uniqueID = rownames(.),
         cvID = sample(round(nrow(fishnet) / 24), 
                       size=nrow(fishnet), replace = TRUE))

ggplot() +
  geom_sf(data = crime_net, aes(fill = countDumping), color = NA) +
  scale_fill_viridis() +
  labs(title = "Count of Illegal Dumping for the fishnet") +
  mapTheme()
