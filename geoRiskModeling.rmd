...
vars_net <- vars_net %>%
    mutate(Abandoned_Cars.nn = nn_function(st_c(st_coid(vars_net)), 
                                           st_c(abandonedCars),
                                           k = 3)) %>%
    mutate(Illegal_Dumping.nn = nn_function(st_c(st_coid(vars_net)), 
                                           st_c(Illegal_Dumping),
                                           k = 3)) %>%
    mutate(liquourLicenses.nn = nn_function(st_c(st_coid(vars_net)), 
                                           st_c(liquourLicenses),
                                           k = 3)) %>%
    mutate(metroStops.nn = nn_function(st_c(st_coid(vars_net)), 
                                           st_c(metroStops),
                                           k = 3)) %>%
    mutate(streetlightsOut.nn = nn_function(st_c(st_coid(vars_net)), 
